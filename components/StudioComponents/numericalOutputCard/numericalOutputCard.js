...
function NumericalOutputCard(props) {
    ...
    function handleNumericalOutputCardChanges(state, action) {
        switch (action.type) {
            ...
            case "FORMULA":
                const updatedState_f = {
                    ...state,
                    formula: action.value
                };

                return updatedState_f;
            case "EDIT":
            ...
            <div className="transition ease-in-out duration-500 w-full none:shadow hover:shadow-md border	px-2 rounded-sm text-blue-900 text-lg py-2 mb-2 select-none">
                ...
                }
                {
                    props.cardElement.availableVariables && props.cardElement.availableVariables.map((item) => {
                        return (
                            <div onClick={(e) => addValueToFormula(`[${item.questionText}]`)} className="inline-block w-5/12 bg-gray-100 text-center border-2 border-blue-900 my-1 rounded hover:bg-blue-900 hover:text-white cursor-pointer m-2 p-2 text-sm">
                                {item.questionText}
                            </div>
                        );
                    })
                ...
                <textarea
                    ...
                    className={
                        state.editMode
                            ? "height-fit-text w-full border p-1.5 border-gray-300 rounded text-sm"
                            : "h-20 w-full border p-1.5 border-gray-300 rounded cursor-not-allowed bg-gray-100 text-sm"
                    }
