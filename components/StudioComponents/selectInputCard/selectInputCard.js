...
function SelectInputCard(props) {
    ...
    function handleoptionsObjectChanges(state, action) {
        ...
        case "MAX_SELECTIONS":
            const max_selection_value = Number.isNaN(parseInt(action.value))
                ? 1
                : parseInt(action.value);
            const updatedState_max_selections = {
                type: state.type,
                name: state.name,
                uuid: state.uuid,
                questionText: state.questionText,
                previewModeDisplay: state.previewModeDisplay,
                maxSelectionVal: max_selection_value,
                optionsObject: state.optionsObject,
                editMode: state.editMode,
            };
          ...
          <div className="transition ease-in-out duration-500 w-full none:shadow hover:shadow-md border	px-2 rounded-sm text-blue-900 text-lg py-2 mb-2 select-none">
            ...
            <div className="flex flex-row h-9 my-2">
              <div className="w-1/2 text-lg text-blue-900">
                {" "}
                {props.elementIndex} | Select Input Card
              </div>
              {/* Edit / Delete Buttons */}
                    ...
                    <>
                      {Object.keys(state.optionsObject).length > 0 ? (
                          ...
                          <span>
                            ...
                            <span>No options has been added !</span>
                          </span>
                      )}
                    </>
                )}
                ...
                {Object.keys(state.optionsObject).map((item, index) => {
                    return (
                        <OptionElement
                          ...
                          elementValue={state.optionsObject[item]}
                          editFunction={optionElementTextFunction}
                          deleteFunction={() =>
              ...
              <div className=" flex flex-row justify-between mb-1.5">
                {state.editMode ? (
                    <div className="w-1/5 h-9 ml-1.5">
                      <div className="text-blue-900 text-xs w-max">
                        Max options users can select:
                      </div>
                      <input
                        className={
                            state.editMode
                                ? "text-sm border-1 rounded px-1 w-32 border-blue-900 py-0.5"
                                : "text-sm border rounded px-1 w-32 border-opacity-50 py-0.5 cursor-not-allowed"
                        }
                        readOnly={!state.editMode}
                        type="number"
                        onKeyDown={event => {
            ...
            {state.editMode ? (
                <div className="text-sm mx-1.5 text-blue-900 mt-2.5">
                  <span className="font-bold">NOTE:</span> The default value for the
                  maximum number of option(s) that the users can select is 1 and this
