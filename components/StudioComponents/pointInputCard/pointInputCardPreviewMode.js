...
function PointInputCardPreviewMode(props) {
        ...
        if (!array.some(option => option.optionUUID === currentOption.optionUUID)) {
            if (array.length === parseInt(props.cardElement.maxSelectionVal)) {
                ...
                if (props.cardElement.outputAssociation) {
                    props.inputCardtotalScoreUpdateMethod(currentOption.optionScore, "ADD", props.cardElement.outputAssociationElement.uuid);
                }
    ...
    function removeOptionFromOptionsSelectedArray(UUID, score, array) {
        ...
        if (array.some(option => option.optionUUID === optionToBeRemoved.optionUUID)) {
            ...
            if (props.cardElement.outputAssociation) {
                props.inputCardtotalScoreUpdateMethod(optionToBeRemoved.optionScore, "SUBTRACT", props.cardElement.outputAssociationElement.uuid);
            }
                    ...
                    <div key={keyName} className='w-full h-9 mb-1 bg-blue-100 border-1 border-blue-400 rounded flex flex-row justify-between'>
                        <div className="flex w-full">
                            ...
                            {(props.cardElement.maxSelectionVal > 1) ?
                                (<>
                                    <div className="font-bold px-1 text-lg text-blue-800 overflow-x-hidden w-2/3 m-0.5 mr-1.5 rounded">{props.cardElement.optionsObject[keyName].optionText}</div>
                                    <div className="flex text-sm my-1 w-1/3 border-2 border-blue-900 mr-3 rounded">
                                        ...
                                        <div className={optionsSelectedArray.some(option => option.optionUUID === keyName) ? optionSelectedStyleString : optionNotSelectedStyleString}
                                            ...
                                            <span className="pt-px">{props.cardElement.optionsObject[keyName].optionScore > 0 ?
                                                (<>+{props.cardElement.optionsObject[keyName].optionScore}</>) :
                                                (<>{props.cardElement.optionsObject[keyName].optionScore}</>)}</span>
                                        </div>
                                    </div>
                                </>) :
                                (<>
                                    <div className="font-bold text-lg text-blue-800 overflow-x-auto w-4/5 m-0.5 px-1">{props.cardElement.optionsObject[keyName].optionText}</div>
                                    <div className="flex">
                                        <div className="px-1.5 py-0.5 m-1">
                                            <input type="radio" className="cursor-pointer" name={props.cardElement.uuid}
                                                onClick={() => {
                                                    ...
                                                    props.inputCardtotalScoreUpdateMethod(props.cardElement.optionsObject[keyName].optionScore - totalScore, 'ADD', props.cardElement.outputAssociationElement.uuid);
                                                }}></input>
                                        </div>
                                        <div className="text-blue-900 border-1 rounded border-blue-900 m-1 px-1 bg-blue-200 font-medium w-12 text-center">
                                            <span className="ml-0.5">{props.cardElement.optionsObject[keyName].optionScore > 0 ?
                                                (<>+{props.cardElement.optionsObject[keyName].optionScore}</>) :
                                                (<>{props.cardElement.optionsObject[keyName].optionScore}</>)}</span>
                                        </div>
                                    </div>
