...
function PointInputCard(props) {
  ...
  const [state, updateOptionsObject] = useReducer(
    handleoptionsObjectChanges,
    initialState
  )
  const [existingCardDialogBoxState, setExistingCardDialogBoxState] =
    useState(false)
  const [
    existingCardDialogBoxSelectionState,
    setExistingCardDialogBoxSelectionState,
  ] = useState(null)
    ...
    switch (action.type) {
      case "QUESTION_TEXT":
        const updatedState_question_text = {
          ...
          optionsObject: state.optionsObject,
          editMode: state.editMode,
          maxSelectionVal: state.maxSelectionVal,
          outputAssociation: state.outputAssociation,
          outputAssociationElement: state.outputAssociationElement,
        }
      ...
      case "EDIT_OPTION_SCORE":
        state.optionsObject[optionElementUUID].optionScore = Number.isNaN(
          parseInt(action.content)
        )
          ? 1
          : parseInt(action.content)
        return {
      ...
      case "MAX_SELECTIONS":
        ...
        return {
          ...
          optionsObject: state.optionsObject,
          editMode: state.editMode,
          maxSelectionVal: max_selection_new_value,
          outputAssociation: state.outputAssociation,
          outputAssociationElement: state.outputAssociationElement,
        }
      ...
      case "OUTPUT_ASSOCIATION_ELEMENT":
        return {
          ...
          maxSelectionVal: state.maxSelectionVal,
          outputAssociation: state.outputAssociation,
          outputAssociationElement: action.value,
        }
      case "CLEAR":
        return {
          type: state.type,
  ...
  function outputAssociationHelper(actionType, cardUUID) {
    if (actionType === "NEW") {
      if (
        state.outputAssociationElement === null ||
        state.outputAssociationElement.type === "EXISTING"
      ) {
        const elementUUID = cardUUID
        props.outputAssociation(
          "output",
          "NumericalOutputComponent",
          elementUUID
        )
      } else {
        alert(
          "Point Input Card: An output card has already been added for output association !"
        )
      }
      ...
      if (props.outputsListCopy.length > 0) {
        ...
        alert(
          "Point Input Card: There are no existing numerical output cards to select from !"
        )
      }
    }
  }
    ...
    return state.editMode ? (
      <>
        {state.previewModeDisplay ? (
          <FontAwesomeIcon
            icon={faEye}
            onClick={() => updateOptionsObject({ type: "PREVIEW_MODE" })}
  ...
  return (
    <>
      ...
      <Modal
        show={existingCardDialogBoxState}
        onHide={() => {
      ...
      >
        ...
        <Modal.Body className="text-blue-900">
          ...
          <div className="mt-2">
            <DropdownButton
              ...
              title={
                existingCardDialogBoxSelectionState
                  ? existingCardDialogBoxSelectionState.display
                  : "Select Element "
              }
            >
                ...
                <Dropdown.Item
                  ...
                  onClick={() => {
                    setExistingCardDialogBoxSelectionState({
                      ...
                      display: currentItem.outputHeading,
                    })
                  }}
                >
        ...
        <Modal.Footer>
          ...
          <Button
            ...
            onClick={() => {
              updateOptionsObject({
                type: "OUTPUT_ASSOCIATION_ELEMENT",
                value: {
                  type: "EXISTING",
                  uuid: existingCardDialogBoxSelectionState.uuid,
                  display: existingCardDialogBoxSelectionState.display,
                },
              })
              setExistingCardDialogBoxState(false)
            }}
      ...
      <div className="w-full border shadow-md p-2 hover:shadow-xl mb-2">
        ...
        <div className="flex flex-row h-9 my-2">
          <div className="w-1/2 text-lg text-blue-900">
            {" "}
            {props.elementIndex} | Point Input Card
          </div>
          {/* Edit / Delete Buttons */}
            ...
            {state.editMode ? (
              <>
                <div
                  ...
                  onClick={() => {
                    updateOptionsObject({ type: "CLEAR" })
                  }}
                >
                  Clear
                </div>
              ...
              <>
                <div
                  className="inline-block w-5/12 text-center border-2 border-blue-900 my-1 rounded hover:bg-blue-900 hover:text-white cursor-pointer"
                  onClick={() => updateOptionsObject({ type: "EDIT" })}
                >
                  Edit
                </div>
                <div
        ...
        <div className="h-9 w-full flex flex-row justify-between">
          <input
            className={
              state.editMode
                ? "h-5/6 w-11/12 px-1 text-blue-900"
                : "h-5/6 w-11/12 px-1 text-blue-900 cursor-not-allowed"
            }
        ...
        <div className="">
          {state.editMode ? (
            <div className="text-blue-900 text-sm p-1">Options Added:</div>
          ) : (
            <>
              {Object.keys(state.optionsObject).length > 0 ? (
                <div className="text-sm text-blue-900">Options Added:</div>
              ) : (
                <div className="text-blue-900 text-sm">
                  <span className="font-bold">NOTE:</span> No options has been
                  added.
                </div>
              )}
            </>
          ...
          <div className="w-full p-1">
            {Object.keys(state.optionsObject).map((item, index) => {
              return (
                <OptionElement
                  optionElementText={state.optionsObject[item].optionText}
                  ...
                  deleteOptionFunction={() =>
                    updateOptionsObject({ type: "DELETE_OPTION", value: item })
                  }
            ...
            <>
              <div className=" flex flex-row justify-end mb-1.5 justify-between">
                <div>
                  <div className="text-blue-900 text-xs">
                    Max options users can select:
                  </div>
                  <input
                    className="border-1 rounded px-1 w-32 border-blue-900 h-6 w-32 text-sm"
                    type="number"
                    min="1"
                    value={state.maxSelectionVal}
                    readOnly={!state.editMode}
                    onKeyDown={event => {
                ...
                <div
                  className="bg-blue-500 p-2 text-center text-white cursor-pointer hover:bg-blue-600 rounded mr-1"
                  onClick={() =>
                    updateOptionsObject({ type: "ADD_OPTION", value: uuidv4() })
                  }
                >
              ...
              {state.outputAssociation && (
                <div className="flex justify-between px-0.5 mb-1">
                  <div className="text-blue-900 pt-2.5 ">
                    Associate card output with:
                  </div>
                  <div className="">
                    <DropdownButton
                      className="w-full"
                      id="dropdown-item-button"
                      title={
                        state.outputAssociationElement
                          ? state.outputAssociationElement.display
                          : "Select Element "
                      }
                    >
                      <Dropdown.Item
              ...
              <div className="text-blue-900 text-sm mt-1.5">
                <div>
                  <span className="font-bold">NOTE:</span> The card output can
                  be associated with only one of the output category cards.
                </div>
                <div className="mt-1">
                  <span className="font-bold">NOTE:</span> A score must be
                  provided for each option and it must be greater than or equal
                  to 0. The default value for the maximum number of option(s)
                  that the users can select is 1 and this value must be greater
                  than 0.
                </div>
              </div>
            </>
