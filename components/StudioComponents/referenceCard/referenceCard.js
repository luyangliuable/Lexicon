...
function ReferenceCard(props) {
  const initialState = {
    ...
    referenceName: props.cardElement.referenceName,
    url_link: props.cardElement.url_link,
    previewModeDisplay: props.cardElement.previewModeDisplay,
    editMode: props.cardElement.editMode,
  }
  const [state, updateReferenceCard] = useReducer(
    handleReferenceCardChanges,
    initialState
  )
  ...
  function handleReferenceCardChanges(state, action) {
    switch (action.type) {
      case "REFERENCE_NAME":
        const updatedState_reference_name = {
          type: state.type,
          name: state.name,
          uuid: state.uuid,
          referenceName: action.value,
          url_link: state.url_link,
          previewModeDisplay: state.previewModeDisplay,
          editMode: state.editMode,
        }
        return updatedState_reference_name
      case "URL_LINK":
        const updatedState_url_link = {
          type: state.type,
          name: state.name,
          uuid: state.uuid,
          referenceName: state.referenceName,
          url_link: action.value,
          previewModeDisplay: state.previewModeDisplay,
          editMode: state.editMode,
        }
        return updatedState_url_link
      case "PREVIEW_MODE":
    ...
    return state.editMode ? (
      <>
        {state.previewModeDisplay ? (
          <FontAwesomeIcon
            icon={faEye}
            onClick={() => updateReferenceCard({ type: "PREVIEW_MODE" })}
      ...
      <div className="w-full border shadow-md p-2 hover:shadow-xl mb-2">
        ...
        <div className="flex flex-row h-9 my-2">
          <div className="w-1/2 text-lg text-blue-900">
            {" "}
            {props.elementIndex} | Reference Card
          </div>
          {/* Edit / Delete Buttons */}
            ...
            {state.editMode ? (
              <>
                <div
                  ...
                  onClick={() => {
                    updateReferenceCard({ type: "CLEAR" })
                  }}
                >
                  Clear
                </div>
              ...
              <>
                <div
                  className="inline-block w-5/12 text-center border-2 border-blue-900 my-1 rounded hover:bg-blue-900 hover:text-white cursor-pointer"
                  onClick={() => updateReferenceCard({ type: "EDIT" })}
                >
                  Edit
                </div>
                <div
        ...
        <input
          className={
            state.editMode
              ? "h-10 w-full border p-1.5 border-gray-300 rounded"
              : "h-10 w-full border p-1.5 border-gray-300 rounded cursor-not-allowed"
          }
          readOnly={!state.editMode}
          value={state.url_link}
          placeholder="Enter URL / Link ..."
          onChange={event =>
            updateReferenceCard({ type: "URL_LINK", value: event.target.value })
          }
        />
        {/* card textarea row */}
      </div>
